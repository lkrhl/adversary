...
### Changed
- Pipeline subprocesses now run on Opus (`--model opus`) instead of Fable 5, which was retired. Uses the `opus` alias rather than a pinned model id so it tracks the current Opus and won't break if a specific version is pulled.

## [2.4.2] - 2026-06-10

### Changed
- Pipeline subprocesses now run on Fable 5 (`--model claude-fable-5`). Previously no `--model` was passed, so stages ran on the caller's default model; the `model: opus` protocol frontmatter was never applied.
